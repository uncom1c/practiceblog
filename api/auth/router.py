...
async def me_handler(user: Annotated[user_base, Depends(get_current_user)], db: AsyncSession = Depends(get_async_session)):
    likes = await user_liked_posts(user.id, db)
    return user_me(id=user.id, email=user.email, username= user.username, liked_posts= likes)
...
async def login_for_access_token(
    ...
    try:
        ...
        raise HTTPException(
            ...
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Произошла ошибка какая-то. {_ex}"
        )
